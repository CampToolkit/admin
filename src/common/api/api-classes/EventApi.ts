import { BaseApi } from "@/common/api/api-classes/BaseApi.ts";
import type {
  Event,
  Event_Coach,
  Event_Group,
} from "@/common/api/event/EventApi.type.ts";
import type {
  AddGroupDto,
  AppointCoachDto,
  CreateLessonDto,
...
export class ClassEventApi extends BaseApi<
  Event,
  CreateLessonDto,
  UpdateLessonDto
> {
  ...
  async updateCoach(lessonCoachId: number, dto: UpdateCoachDto) {
    const { data } = await axiosConfig.patch<Event_Coach>(
      `lesson-coach/${lessonCoachId}`,
      dto,
    );
    return data;
  }
